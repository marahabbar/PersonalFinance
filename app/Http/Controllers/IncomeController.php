...
class IncomeController extends Controller
{
   ...
   public function income_update(int $id, Request $request)
   {
      ...
      return response()->json([
         "income" => $this->income_show($id), "user" =>
   ...
   public function income_store(Request $request)
   {
      ...
      // return new balance.........                    
      return response()->json([
         "income" => new  IncomeRresource($income), "user" => User::where('id', $income->user_id)->select('balance', 'total_saving_amount', 'total_incomes')->get(),
   ...
   public function income_delete(int $id)
   {
      ...
      return response()->json([
         "user" => User::where('id', $deleted_income->user_id)->select('balance', 'total_saving_amount', 'total_incomes')->get(),
...
// monthly/daily income
   public function income_add(int $id)
   {
      ...
      if($monthly_exp->monthly==1)
      $notif->expense_notif($income ->id,"income","montly");
      elseif($monthly_exp->monthly==2)
        $notif->expense_notif($income ->id,"income","weekly");
      else
      $notif->expense_notif($income ->id,"income","daily");
