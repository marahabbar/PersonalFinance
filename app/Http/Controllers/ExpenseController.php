...
class ExpenseController extends Controller
{
    ...
    public function expense_update(int $id, Request $request)
    {
        ...
        $notif->balance_notif($request->user_id);
        //return balance && total expenses.........

        return response()->json([
            $this->expense_show($id),
    ...
    public function expense_delete(int $id)
    {
        ...
        return response()->json([
            User::where('id', $deleted_expense->user_id)->select('balance', 'total_expenses')->get(),
            Category::where('id', $deleted_expense->category_id)->select('current_amount')->get()
        ]);
    }


//monthly /daily expense
    public function expense_add(int $id)
    {
       ...
       if($monthly_exp->monthly==1)
         $notif->expense_notif($expense ->id,"expense","montly");
       elseif($monthly_exp->monthly==2)
         $notif->expense_notif($expense ->id,"expense","weekly");
       else
       $notif->expense_notif($expense ->id,"expense","daily");
         // return new balance........
